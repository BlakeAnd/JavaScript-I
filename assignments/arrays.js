...
for(j = 0; j < inventory.length; j++){
    carYears[j] = inventory[j].car_year;
}
carYears.sort();
...
let oldCars = [];
for(k = 0; k < carYears.length; k++){
    if(carYears[k] >= 2000){ break; }
    oldCars[k] = carYears[k];
}
...
let BMWAndAudi = [];
for(l = 0; l < inventory.length; l++){
    if(inventory[l].car_make === 'BMW' ||  inventory[l].car_make=== 'Audi'){ 
        BMWAndAudi.push(inventory[l]);
    }
